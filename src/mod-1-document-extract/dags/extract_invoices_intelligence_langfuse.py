"""
## Invoice Intelligence Extraction Pipeline with LangChain + Langfuse

This DAG processes PDF invoices from the MinIO landing-zone/invoices bucket and extracts 
structured data using LangChain's document processing and extraction chains with 
automatic Langfuse observability.
...
- Automatic LLM observability via Langfuse-wrapped OpenAI client
- Structured data storage in PostgreSQL

Key Features:
- ✅ Automatic OpenAI call monitoring and token tracking
- ✅ Zero-configuration tracing of all LLM interactions
- ✅ Cost analysis and performance monitoring in Langfuse dashboard
- ✅ Structured output validation with Pydantic schemas

![Invoice Intelligence Pipeline](https://via.placeholder.com/800x400/2196F3/FFFFFF?text=LangChain+Langfuse+Invoice+Pipeline)
"""
...
langfuse = Langfuse(
    secret_key="sk-lf-ade4ac4f-f5ef-4374-ab7e-ce9575e70aa0",
    public_key="pk-lf-9d9c547c-1657-4126-ace1-ff8e826d546c", 
...
@dag(
    ...
    tags=["intelligence", "invoice", "llm", "langchain", "openai", "langfuse"],
)
def extract_invoices_intelligence_langfuse():
    ...
    def extract_invoice_data_batch_langchain(batch_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        def setup_extraction_llm():
            """Setup Langfuse-enabled OpenAI client with structured output"""
            # Initialize Langfuse-wrapped OpenAI client (automatic tracing)
            client = OpenAI(api_key=openai_api_key)
            
            # Use a structured prompt that explains the expected JSON format
            prompt_text = """You are an expert invoice data extraction system. Extract structured information from this invoice text and return a valid JSON object.

IMPORTANT: Follow this exact JSON structure. Use null for any field not found.

CRITICAL NOTES:
- For "modifiers" arrays: each modifier MUST have "name", "value", and "extra_cost" fields
- Use empty array [] if no modifiers exist for a line item
- All nested objects must follow the exact structure shown below

Expected JSON Structure:
{{
    "invoice_number": "string or null",
    "invoice_type": "standard|uber_eats|delivery|receipt|proforma|credit_note|debit_note",
    ...
    "customer": {{
        ...
        "tax_info": {{ "same structure as vendor tax_info" }}
    }},
    "line_items": [
        {{
            ...
            "modifiers": [
                {{
                    "name": "modifier name (e.g., 'Extra cheese', 'Side dish')",
                    "value": "modifier value or null (e.g., 'Gruyère', 'Mashed potatoes')",
                    "extra_cost": 2.50
                }}
            ],
    ...
    "payment_summary": {{
        ...
        "amount_paid": 117.00,
        "amount_due": 0.00,
        "currency": "USD|EUR|BRL|GBP|JPY|CAD"
    }},
    "payment_details": {{
        "method": "credit_card|debit_card|cash|pix|bank_transfer|check|other",
...
Return ONLY the JSON object, no additional text."""
        ...
        def extract_from_chunks(client, prompt_text, chunks: List[str], filename: str) -> GeneralInvoice:
            """Extract data from document chunks with automatic Langfuse tracing"""
            ...
            try:
                ...
                # Format prompt with invoice text
                formatted_prompt = prompt_text.format(invoice_text=combined_text)
                
                # Make OpenAI call with automatic Langfuse tracing
                ...
                print(f"🔍 Raw OpenAI response for {filename}:")
                print(f"    {raw_response[:500]}...")  # First 500 chars for debugging
                
                raw_data = json.loads(raw_response)
                invoice_model = GeneralInvoice(**raw_data)
                return invoice_model
                
            except json.JSONDecodeError as json_error:
                print(f"⚠️ JSON parsing error for {filename}: {json_error}")
                print(f"    Raw response: {response.choices[0].message.content[:300]}...")
                return None
            except Exception as chunk_error:
                print(f"⚠️ Pydantic validation error for {filename}: {chunk_error}")
                # Try to show what data caused the validation error
                try:
                    raw_data = json.loads(response.choices[0].message.content)
                    print(f"    Raw data keys: {list(raw_data.keys()) if isinstance(raw_data, dict) else 'Not a dict'}")
                    if 'line_items' in raw_data and raw_data['line_items']:
                        print(f"    Line items count: {len(raw_data['line_items'])}")
                        if raw_data['line_items'][0].get('modifiers'):
                            print(f"    First modifier sample: {raw_data['line_items'][0]['modifiers'][0]}")
                except:
                    pass
                return None
        
        def extract_single_document_langchain(doc_info: Dict[str, Any], client, prompt_text) -> Dict[str, Any]:
            ...
            try:
                print(f"🔍 Extracting data from {filename} using Langfuse OpenAI client...")
                
                # If document has chunks, use chunk-based extraction
                if chunks:
                    invoice_model = extract_from_chunks(client, prompt_text, chunks, filename)
                else:
                    # Fallback: use full text (truncated if necessary)
                    truncated_text = full_text[:8000] if len(full_text) > 8000 else full_text
                    
                    # Format prompt with invoice text
                    formatted_prompt = prompt_text.format(invoice_text=truncated_text)
                    ...
                    response = client.chat.completions.create(
                        ...
                        metadata={
                            ...
                            "text_truncated": len(full_text) > 8000
                        }
                    )
                    
                    # Parse response and create Pydantic model
                    raw_response = response.choices[0].message.content
                    print(f"🔍 Raw OpenAI response (fallback) for {filename}:")
                    print(f"    {raw_response[:500]}...")  # First 500 chars for debugging
                ...
                invoice_model.raw_data = {"langfuse_extraction": True, "chunk_count": len(chunks)}
                
                # Convert to flat dictionary
                flat_data = invoice_model.to_flat_dict()
                
                print(f"✅ Successfully extracted and validated data from {filename}")
                print(f"   - Confidence: {flat_data.get('extraction_confidence', 0.0):.2f}")
                print(f"   - Fields extracted: {len([v for v in flat_data.values() if v is not None])}")
        ...
        try:
            # Setup Langfuse-enabled extraction components
            client, prompt_text = setup_extraction_llm()
            
            # Process all documents in batch
            extracted_data = []
            for doc_info in documents:
                start_time = time.time()
                result = extract_single_document_langchain(doc_info, client, prompt_text)
                end_time = time.time()
            ...
            successful_count = len([d for d in extracted_data if d.get("status") == "success"])
            print(f"📊 Batch {batch_id} Summary:")
            print(f"   - Total processed: {len(extracted_data)}")
            print(f"   - Successful: {successful_count}")
            print(f"   - Success rate: {successful_count / len(extracted_data) * 100:.1f}%" if extracted_data else "   - Success rate: 0%")
    ...
    def generate_extraction_report(
        ...
        report = {
            "pipeline": "extract_invoices_intelligence_langfuse",
...
# Create DAG instance
dag_instance = extract_invoices_intelligence_langfuse()